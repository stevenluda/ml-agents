...
class PPOTrainer(Trainer):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, seed):
        ...
        self.use_vector_2D_obs = (env.brains[brain_name].vector_observation_2D_length > 0)
    ...
    def take_action(self, all_brain_info: AllBrainInfo):
        ...
        if self.use_vector_2D_obs:
            feed_dict[self.model.vector_2D_in] = curr_brain_info.vector_observations_2D
    ...
    def construct_curr_info(self, next_info: BrainInfo) -> BrainInfo:
        ...
        for agent_id in next_info.agents:
            ...
            vector_observations_2D.append(agent_brain_info.vector_observations_2D[agent_index])
    ...
    def generate_intrinsic_rewards(self, curr_info, next_info):
        ...
        if self.use_curiosity:
            ...
            if self.use_vector_2D_obs:
                feed_dict[self.model.vector_2D_in] = curr_info.vector_observations_2D
                feed_dict[self.model.next_vector_2D_in] = next_info.vector_observations_2D
            if self.use_recurrent:
    ...
    def generate_value_estimate(self, brain_info, idx):
        ...
        if self.use_vector_2D_obs:
            feed_dict[self.model.vector_2D_in] = [brain_info.vector_observations_2D[idx]]
        if self.use_recurrent:
        ...
        for agent_id in next_info.agents:
            ...
            if stored_info is not None:
                ...
                if not stored_info.local_done[idx]:
                    ...
                    if self.use_vector_2D_obs:
                        self.training_buffer[agent_id]['vector_2D_obs'].append(stored_info.vector_observations_2D[idx])
                        self.training_buffer[agent_id]['next_vector_2D_in'].append(
                            next_info.vector_observations_2D[next_idx])
                    if self.use_recurrent:
    ...
    def update_model(self):
        ...
        for k in range(num_epoch):
            ...
            for l in range(len(self.training_buffer.update_buffer['actions']) // n_sequences):
                ...
                if self.use_vector_2D_obs:
                    # test = np.array(buffer['vector_2D_obs'][start:end]).reshape(
                    #     [-1, self.brain.vector_observation_2D_length, self.brain.vector_observation_2D_width, 1])
                    feed_dict[self.model.vector_2D_in] = np.array(buffer['vector_2D_obs'][start:end]).reshape(
                        [-1, self.brain.vector_observation_2D_length, self.brain.vector_observation_2D_width, 1])
                    if self.use_curiosity:
                        feed_dict[self.model.next_vector_2D_in] = np.array(buffer['next_vector_2D_in'][start:end]) \
                            .reshape([-1, self.brain.vector_observation_2D_length, self.brain.vector_observation_2D_width, 1])
