...
class BrainInfo:
    def __init__(self, visual_observation, vector_observation, vector_observation_2D, text_observations, memory=None,
                 reward=None, agents=None, local_done=None,
                 vector_action=None, text_action=None, max_reached=None, vector_action2=None):
        """
        Describes experience at current step of all agents linked to a brain.
        """
        self.visual_observations = visual_observation
        self.vector_observations = vector_observation
        self.vector_observations_2D = vector_observation_2D
        ...
        self.previous_vector_actions2 = vector_action2
...
class BrainParameters:
    def __init__(self, brain_name, brain_param):
        ...
        self.vector_observation_2D_length = brain_param["vectorObservation2DLength"]
        self.vector_observation_2D_width = brain_param["vectorObservation2DWidth"]
    ...
    def __str__(self):
        ...
        Vector Observation 2D length (per agent): {4}
        Vector Observation 2D width (per agent): {5}
        Number of stacked Vector Observation: {6}
        Vector Action space type: {7}
        Vector Action space size (per agent): {8}
        Vector Action descriptions: {9}'''.format(self.brain_name,
                                                  str(self.number_visual_observations),
                                                  self.vector_observation_space_type,
                                                  str(self.vector_observation_space_size),
                                                  str(self.vector_observation_2D_length),
                                                  str(self.vector_observation_2D_width),
